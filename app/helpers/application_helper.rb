module ApplicationHelper
    ...
    def get_specific_accepted_request(user_id)
    ...
    def friend?(user_id)
       get_specific_accepted_request(user_id).present?
    end
    
    def received_or_sent_request?(user_id)
        Request.where("(receiver_id = #{current_user.id} and sender_id = #{user_id}) or (receiver_id = #{user_id} and sender_id = #{current_user.id}) and status = 'p'").present?
    end
    
    def request_sent?(user_id)
        Request.where("(receiver_id = #{user_id} and sender_id = #{current_user.id}) and status = 'p'").present?
    end
    
    def request_received(user_id)
